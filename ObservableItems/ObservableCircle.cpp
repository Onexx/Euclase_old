...
void ObservableCircle::DirectionDrawing(Direction &dir){
    ...
    switch ( dir ){
    case(Right) : {dirline->setLine(this->rect().width(), this->rect().height()/2, this->rect().width()+7, this->rect().height()/2);break;}
    case(Down)  : {dirline->setLine(this->rect().width()/2, this->rect().height(), this->rect().width()/2, this->rect().height()+7);break;}
    case(Left)  : {dirline->setLine(0, this->rect().height()/2, -7, this->rect().height()/2);break;}
    case(Up)    : {dirline->setLine(this->rect().width()/2, 0, this->rect().width()/2, -7);break;}
    }
